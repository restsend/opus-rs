...
fn main() {
    ...
    for f in 0..num_frames {
        ...
        let bytes = encoder.encode(&input, frame_size, &mut compressed).expect("Encoding failed");
        if f == 0 {
            println!("Encoded first frame: {} bytes, TOC={:02X}", bytes, compressed[0]);
        }

        let mut output = vec![0.0f32; frame_size];
        decoder.decode(&compressed[..bytes], frame_size, &mut output).expect("Decoding failed");
    ...
    for delay in (0..frame_size + overlap).step_by(1) {
        ...
        if total_output.len() <= delay { continue; }
