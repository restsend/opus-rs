...
fn test_bands_match() {
    ...
    let ref_energies = [
        0.000000, 0.149748, 0.297984, 0.443214, 0.583972, 0.718839, 0.846456, 0.965538,
        1.591308, 1.835139, 2.005610, 2.095897, 2.919261, 2.482801, 1.667935, 0.692719,
        1.801952, 3.756825, 3.890290, 3.614069, 4.083487
    ];
    ...
    for i in 0..ref_energies.len() {
        ...
        assert!(diff < 1e-5, "Band {} energy mismatch: expected {}, got {}", i, ref_energies[i], band_e[i]);
    }
    ...
    let ref_norm_head = [
        0.000000, 1.000000, 1.000000, 1.000000, 1.000000, 1.000000, 1.000000, 1.000000, 0.675474, 0.737384
    ];
    
    for i in 0..10 {
        let diff = (x_norm[i] - ref_norm_head[i]).abs();
        assert!(diff < 1e-5, "Norm index {} mismatch: expected {}, got {}", i, ref_norm_head[i], x_norm[i]);
    }
}
