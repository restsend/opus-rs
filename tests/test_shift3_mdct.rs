...
fn test_mdct_shift3() {
    ...
    eprintln!("Testing MDCT with shift={}, n={}, overlap={}", shift, n, overlap);
    ...
    eprintln!("Input: {} samples, first 10: {:?}", input_len, &input[..10.min(input_len)]);
    ...
    eprintln!("MDCT output: {} coeffs, max={}", freq.len(), freq.iter().map(|f| f.abs()).fold(0.0f32, f32::max));
    ...
    eprintln!("MDCT backward output: {} samples, max={}", output.len(), output.iter().map(|f| f.abs()).fold(0.0f32, f32::max));
    ...
    if reconstruct_start < output.len() {
        ...
        eprintln!("Reconstructed samples starting at {}: {} available", reconstruct_start, available);
        eprintln!("First 10 reconstructed: {:?}", &output[reconstruct_start..reconstruct_start+10.min(available)]);
