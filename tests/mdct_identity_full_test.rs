...
fn test_mdct_identity_full() {
    ...
    let mut all_in = vec![0.0f32; 3 * frame_size];
    for i in 0..all_in.len() {
        all_in[i] = (i as f32 * 0.1).sin();
    }

    // Decoder history buffer (simulated)
    let mut decode_mem = vec![0.0f32; frame_size + overlap];

    let mut all_out = Vec::new();

    // Frame 0
    {
        let frame_in = &all_in[0..frame_size + overlap];
    ...
    // Frame 1
    {
        // For TDAC, consecutive encoder frames step by frame_size.
        // Frame 0 uses all_in[0..frame_size+overlap] = [0..1080]
        // Frame 1 uses all_in[frame_size..2*frame_size+overlap] = [960..2040]
        // The overlap is the shared region: all_in[960..1080]
        let frame_in = &all_in[frame_size..frame_size + frame_size + overlap];
        let mut freq = vec![0.0f32; frame_size];
        mdct.forward(frame_in, &mut freq, &window, overlap, 0, 1);
